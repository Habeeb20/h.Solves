  ...
  //edit submit
    ...
    useEffect(() => {
        ...
        const getCandidateDetails = async() => {
            try {
                const token = localStorage.getItem("token")
                const response = await axios.get(`${import.meta.env.VITE_API_3}/jobcandidates/${id}`, {
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
                })
  ...
  <div className="bg-white shadow-lg rounded-lg p-6">
    ...
    <div className="flex flex-col md:flex-row items-center justify-between gap-4">
      <div className="flex items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-600">{viewCandidate.jobseekerId?.phone}</p>
          <p className="text-sm text-purple-600">{viewCandidate.jobseekerId?.email}</p>
        </div>
      </div>
    ...
    <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
      {/* Left Column */}
        ...
        <h4 className="text-center text-lg font-semibold">About the Candidate</h4>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
        <div>
            <h4 className="text-sm font-medium text-gray-700">full Name</h4>
            <p className="text-sm text-blue-500"> {viewCandidate.jobseekerId?.fname} {viewCandidate.jobseekerId?.lname}</p>
          </div>
          <div>
  ...
  <div className="bg-white shadow-lg rounded-xl p-6">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
      <div>
        ...
        <p className="text-sm sm:text-base text-blue-500 font-medium">{viewCandidate?.jobId?.companyName}</p>
      </div>

      <div>
        <h4 className="text-sm font-medium text-gray-700">About the Company</h4>
        <p className="text-sm sm:text-base text-blue-500">{viewCandidate.jobId?.companyAbout}</p>
      </div>

      <div>
        <h4 className="text-sm font-medium text-gray-700">Job Title</h4>
        <p className="text-sm sm:text-base text-blue-500">{viewCandidate.jobId?.jobTitle}</p>
      </div>

      <div>
        <h4 className="text-sm font-medium text-gray-700">Job Type</h4>
        <p className="text-sm sm:text-base text-blue-500">{viewCandidate.jobId?.jobType}</p>
      </div>

      <div>
        <h4 className="text-sm font-medium text-gray-700">Job Requirements</h4>
        <p className="text-sm sm:text-base text-blue-500">{viewCandidate.jobId?.requirements}</p>
      </div>

      <div>
        <h4 className="text-sm font-medium text-gray-700">Salary</h4>
        <p className="text-sm sm:text-base text-blue-500">{viewCandidate.jobId?.salary}</p>
      </div>
      <div>
        <h4 className="text-sm font-medium text-gray-700">Location</h4>
        <p className="text-sm sm:text-base text-blue-500">{viewCandidate.jobId?.location}</p>
      </div>
    </div>
  </div>
</section>
    ...
    <div className="flex justify-center mt-6">
      <button className="bg-blue-500 text-white font-semibold rounded-lg px-6 py-2 hover:bg-blue-700 transition">
        Chat the job seeker
      </button>
    </div>
  </div>
          ...
          default:
            return   <div className="pb-20 bg-gray-100 min-h-screen w-full">
            <motion.div
               initial={{ opacity: 0 }}
     animate={{ opacity: 1 }}
     transition={{ duration: 0.5 }}
        ...
        <div className="bg-white shadow-md rounded-lg p-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-lg font-semibold">{viewCandidate.jobId?.companyName}</h3>
                <p className="text-sm text-gray-600">{viewCandidate.jobseekerId?.fname} {viewCandidate.jobseekerId?.lname}</p>
          ...
          <div className="mt-6">
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {error && <p className="text-red-500">{error}</p>}
                <h4 className="text-sm font-medium text-gray-700">Company Name</h4>
                <p className="text-sm text-gray-900">{viewCandidate.companyName}</p>
              </div>
              <div>
                <h4 className="text-sm font-medium text-gray-700">About the Company</h4>
                <p className="text-sm text-gray-900">{viewCandidate.companyAbout}</p>
              </div>
              <div>
                <h4 className="text-sm font-medium text-gray-700">
job Title
                </h4>
                <p className="text-sm text-gray-900"> {viewCandidate.jobTitle}</p>
              </div>
              <div>
                <h4 className="text-sm font-medium text-gray-700">Job type</h4>
                <p className="text-sm text-gray-900">{viewCandidate.jobType}</p>
              </div>
              <div>
                <h4 className="text-sm font-medium text-gray-700">Job Requirements</h4>
                <p className="text-sm text-gray-900">{viewCandidate.requirements}</p>
              </div>
              <div>
                <h4 className="text-sm font-medium text-gray-700">Salary</h4>
                <p className="text-sm text-gray-900">{viewCandidate.salary}</p>
              </div>

              <div>
                <h4 className="text-sm font-medium text-gray-700">Location</h4>
                <p className="text-sm text-gray-900">{viewCandidate.location}</p>
              </div>
              <div>
                <h4 className="text-sm font-medium text-gray-700">years Experience needed</h4>
                <p className="text-sm text-gray-900">{viewCandidate.experience} years</p>
              </div>
              <div>
                <h4 className="text-sm font-medium text-gray-700">Number of vacancies</h4>
                <p className="text-sm text-gray-900">{viewCandidate.vacancies} </p>
              </div>
